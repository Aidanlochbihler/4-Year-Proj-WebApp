...
def grid(request):
    ...
    grid_height = 0.000025 # height
    grid_width = 0.000035 # width
    for trip in trips:
    ...
    for rounded_lat in grid:
        ...
        for rounded_long in grid[rounded_lat]:
            avg_acc = grid[rounded_lat][rounded_long]['accsum']/grid[rounded_lat][rounded_long]['count']
            red, green, blue = hsv_to_rgb(min(max(0.35 - avg_acc * 0.04, 0), 0.3), 0.9, 1)
...
def inspect_trip(request, trip_id):
    ...
    context = {'graph_div': graph_div, "cords": points, "colors": colors}
    return render(request, 'webpages/trip.html', context)


def trips(request, sort_by='TripDuration', is_ascending=1):
    is_ascending = bool(is_ascending)

    if not request.user.is_authenticated:
        return redirect('%s?next=%s' % (settings.LOGIN_URL, request.path))

    t = trip_col.find({"user_id": request.user.id}, {"data": 0})
    t = list(t)
    icons = {i: ' ' for i in ['TripDuration', 'Filename', 'StartTime', 'EndTime', 'DateUploaded']}
    ...
    return render(request, 'webpages/trips.html', context)
